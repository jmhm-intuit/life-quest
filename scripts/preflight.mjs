...
try {
  ...
  version.version === '3.6.0' ? pass('version 3.6.0') : fail(`version.json version is ${version.version}`);
  Number(version.schemaVersion) === 18 ? pass('schema 18') : fail(`schema is ${version.schemaVersion}`);
} catch (error) { fail(`invalid version.json: ${error.message}`); }
try {
  const manifest = JSON.parse(fs.readFileSync(path.join(root, 'manifest.webmanifest'), 'utf8'));
  String(manifest.name || '').includes('3.6') ? pass('manifest version label') : fail('manifest name does not include 3.6');
  pass('manifest JSON');
...
for (const marker of ['Questline v3.6.0','app.css','app.js','manifest.webmanifest']) index.includes(marker) ? pass(`index includes ${marker}`) : fail(`index missing ${marker}`);
const app = fs.readFileSync(path.join(root, 'app.js'), 'utf8');
for (const marker of ["const VERSION = '3.6.0'", 'const SCHEMA_VERSION = 18', "const STORAGE_KEY = 'questline-v3-6'"]) app.includes(marker) ? pass(`app includes ${marker}`) : fail(`app missing ${marker}`);
try {
  const sw = fs.readFileSync(path.join(root, 'service-worker.js'), 'utf8');
  sw.includes('questline-v3-6-cache-v1') ? pass('service-worker cache key') : fail('service-worker cache key is not v3.6');
...
console.log('Questline 3.6 preflight complete.');
